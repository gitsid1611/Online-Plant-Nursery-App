...
public class OrderImp implements OrderService{
	...
	public Orders deleteOrder(int BookingOrderId, String user) throws OrderException {
		// TODO Auto-generated method stub
		Optional<Orders> opt=cdao.findById(BookingOrderId);
		if(opt.isEmpty()) {
			Orders exitCust=opt.get();
			cdao.delete(exitCust);
			return exitCust;
		}else {
	...
	public Orders viewOrder(int BookingOrderId, String user) throws OrderException {
		return cdao.findById(BookingOrderId).orElseThrow(() -> new OrderException("Oders does not exist with roll :" +BookingOrderId));
	}
