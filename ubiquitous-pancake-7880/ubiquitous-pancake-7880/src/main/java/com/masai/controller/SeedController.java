...
//	DeleteSeed->
	...
	public ResponseEntity<List<SeedModel>> getSeedByNameHandler(@PathVariable("name") String Name) throws SeedException{
		
		
		List<SeedModel> seed= seedService
				.getSeedBycommonName(Name);
		
		return new ResponseEntity<List<SeedModel>>(seed,HttpStatus.OK);
		
	}
	
	@GetMapping("/showseedsList/{type}")
	public ResponseEntity<List<SeedModel>> getPlantBytypeOfSeedHandler(@PathVariable("type") String type) throws SeedException{
		
		
		List<SeedModel> seeds= seedService.getseedByTypeOfSeed(type);
		
		return new ResponseEntity<List<SeedModel>>(seeds,HttpStatus.OK);
